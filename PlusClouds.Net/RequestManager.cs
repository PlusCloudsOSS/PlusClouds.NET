...
namespace PlusCloudsNet
{
    public static class RequestManager
    {
            ...
            params KeyValuePair<string, object>[] parameters) where TResponse : new()
        {
            ...
            if (typeof (TResponse).GetInterfaces().Contains(typeof (IAuthResponse)))
            {
                client.OnBeforeExecute(request);
            }
            ...
            IRequest request) where TResponse : new()
        {
            ...
            if (tokenizedRequest != null)
            {
                ...
                {
                    if (client.AuthenticateResponse == null ||
                        (!client.AuthenticateResponse.Result &&
                         client.AuthenticateResponse.Expires >= DateTime.Now.Ticks))
            ...
            var userSessionRequest = request as IUserSessionRequest;
            if (userSessionRequest != null)
            {
                if (client.Config.AutoUpdateUserLogin)
                {
                    if (client.Users.UserSession == null ||
                        (client.Users.UserSession.Expiration.ToUtcDateTime() >= DateTime.UtcNow))
                    {
                        client.Users.Authenticate(new UserAuthenticateRequest
                        {
                            Email = client.Config.UserEmail,
                            Password = client.Config.Password
                        });
                    }
                }

                userSessionRequest.SessionId = client.Users.UserSession.Id;
            }
